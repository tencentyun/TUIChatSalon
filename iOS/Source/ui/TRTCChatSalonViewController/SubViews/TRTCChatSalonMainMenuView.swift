...
public typealias CSIconTuple = (normal: UIImage, selected: UIImage)

protocol TRTCChatSalonMainMenuDelegate: class {
    func menuView(menu: TRTCChatSalonMainMenuView, click item: UIButton, index: Int)
}

class TRTCChatSalonMainMenuView: UIView {
    private var isViewReady: Bool = false
    private let icons: [CSIconTuple]
    private var buttons: [UIButton] = []
    weak var delegate: TRTCChatSalonMainMenuDelegate?
    /// 初始化方法
    /// - Parameters:
    ///   - frame: 视图frame
    ///   - icons: 视图菜单icons（最多5个，最少1个）
    init(frame: CGRect = .zero, icons: [CSIconTuple] ) {
        self.icons = icons
...
extension TRTCChatSalonMainMenuView {
    private func createButtons(icons: [CSIconTuple]) -> [UIButton] {
