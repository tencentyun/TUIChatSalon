...
protocol TRTCChatSalonViewResponder: NSObject {
...
//    private(set) var isSeatInitSuccess: Bool = false
    ...
    public var isOwner: Bool {
        ...
        return userID == roomInfo.ownerId // 是否是房主
    }
    ...
    func clickSeat(model: ChatSalonSeatInfoModel) {
    ...
    public func createRoom(toneQuality: Int = 0) {
        ...
        if !coverUrl.hasPrefix("http") {
            coverUrl = dependencyContainer.chatSalonUserInfo?.avatarURL ?? ""
        }
...
extension TRTCChatSalonViewModel: TRTCChatSalonDelegate {
    ...
    public func onWarning(code: Int32, message: String) {
        
    }
    
    public func onDebugLog(message: String) {
        
    }
    
    public func onRoomDestroy(message: String) {
    ...
    public func onAnchorEnterSeat(user: ChatSalonUserInfo) {
    ...
    public func onAnchorLeaveSeat(user: ChatSalonUserInfo) {
        if user.userID == roomInfo.ownerId {
            // 房主下麦就不提醒了
            return;
        }
    ...
    public func onSeatMute(userID: String, isMute: Bool) {
    ...
    public func onAudienceEnter(userInfo: ChatSalonUserInfo) {
    ...
    public func onAudienceExit(userInfo: ChatSalonUserInfo) {
        memberAudienceDic.removeValue(forKey: userInfo.userID)
        handleAudienceListChanged()
    }
    
    public func onUserVolumeUpdate(userVolumes: [TRTCVolumeInfo], totalVolume: Int) {
    ...
    public func onRecvRoomTextMsg(message: String, userInfo: ChatSalonUserInfo) {
    ...
    public func onRecvRoomCustomMsg(cmd: String, message: String, userInfo: ChatSalonUserInfo) {
        
    }
    
    public func onReceiveNewInvitation(identifier: String, inviter: String, cmd: String, content: String) {
        ...
        if roomType == .anchor && roomInfo.ownerId == userId {
    ...
    public func onInviteeAccepted(identifier: String, invitee: String) {
        if roomType == .audience {
            guard let _ = mInvitationSeatDic.removeValue(forKey: identifier) else {
                return
            }
            guard let userId = dependencyContainer.chatSalonUserInfo?.userID else {
                debugPrint("chatSalonUserInfo is nil")
    ...
    public func onInviteeRejected(identifier: String, invitee: String) {
    ...
    public func onInvitationCancelled(identifier: String, invitee: String) {
        if roomType == .anchor {
    ...
    public func onInvitationTimeout(identifier: String) {
        if roomType == .anchor {
            if let userID = self.requestTakeSeatMap.first(where: { $1.invitedId == identifier })?.key {
                self.requestTakeSeatMap.removeValue(forKey: userID)
            }
