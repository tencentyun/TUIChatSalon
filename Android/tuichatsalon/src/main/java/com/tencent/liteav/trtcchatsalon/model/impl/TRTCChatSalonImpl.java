...
public class TRTCChatSalonImpl extends TRTCChatSalon implements ITXRoomServiceDelegate, ChatSalonTRTCServiceDelegate {
    ...
    public void login(final int sdkAppId, final String userId, final String userSig,
                      final TRTCChatSalonCallback.ActionCallback callback) {
    ...
    @Override
    public void createRoom(final int roomId, final TRTCChatSalonDef.RoomParam roomParam,
                           final TRTCChatSalonCallback.ActionCallback callback) {
        runOnMainThread(new Runnable() {
            @Override
            public void run() {
                        ...
                        if (code == 0) {
                            enterTRTCRoomInner(roomId, mUserId, mUserSig, TRTCCloudDef.TRTCRoleAnchor,
                                    new TRTCChatSalonCallback.ActionCallback() {
                                        ...
                                        public void onCallback(final int code, final String msg) {
                                            TRTCLogger.i(TAG,
                                                    "trtc enter room finish, room id:" + roomId + " code:"
                                                            + code + " msg:" + msg);
                                            runOnDelegateThread(new Runnable() {
                                                @Override
                                                public void run() {
        ...
        runOnMainThread(new Runnable() {
            ...
            public void run() {
                ...
                TXRoomService.getInstance().enterRoom(mRoomId, new TXCallback() {
                    ...
                    public void onCallback(final int code, final String msg) {
                        TRTCLogger.i(TAG,
                                "enter room service finish, room id:" + roomId + " code:" + code + " msg:" + msg);
                        runOnMainThread(new Runnable() {
                            @Override
                            public void run() {
                                if (code == 0) {
    ...
    private void muteSeat(final String userId, final boolean isMute,
                          final TRTCChatSalonCallback.ActionCallback callback) {
        runOnMainThread(new Runnable() {
            @Override
            public void run() {
